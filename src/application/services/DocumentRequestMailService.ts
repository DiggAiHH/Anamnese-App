 ...
 *
 * @security Do not include patient data in subject lines.
 */
function getSubjectLine(type: DocumentType, labels: Required<DocumentRequestLabels>): string {
...
/**
 * Build minimal summary for encrypted requests (no PHI).
 */
 ...
 */
async function encryptPayload(request: IDocumentRequest, encryptionKey: string): Promise<string> {
...
async function buildMailBody(
  ...
  try {
    const summary = buildMinimalSummary(request, labels);
    ...
    throw new Error('Encryption failed. Request was NOT sent.');
  }
}
 ...
 * @param patientName Optional patient name for subject line (ignored for privacy)
...
export async function buildDocumentRequestMailtoUri(
  ...
  const subject = getSubjectLine(request.documentType, labels);
