...
const buildKeyManager = (os: 'ios' | 'android' | 'windows') => {
  ...
  let storage: typeof import('@react-native-async-storage/async-storage');
  jest.isolateModules(() => {
  ...
  return {
    keychain,
    storage: storage as typeof import('@react-native-async-storage/async-storage'),
